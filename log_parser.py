...
class CIMA_Commanded_Scan:
    ...
    def __init__(self, loglines, start=0, command_type="request"):
        ...
        for i, line in enumerate(loglines):
            ...
            if line.startswith("LOAD"):
                ...
                try:
                    m = re.match(r"cima_control_puppi_(\d+).conf", line.split()[-1])
                    if m:
                        self._data["frequency"] = int(m.groups()[0])
                    logger.debug("Frequency = {}".format(self._data["frequency"]))
                except ValueError:
                    logger.error(
                        "Unable to parse frequency from line:\n.{}".format(line)
                    )
                    return None
                ...
                if "change_puppi_parfile" in line:
                    ...
                    if "FOLD" in line:
                        self._data["fold_dumptime"], self._data["fold_bins"] = map(
                            int, line.replace('"', "").split()[-2:]
                        )
                    elif "CAL" in line:
                        self._data["cal_dumptime"], self._data["cal_bins"] = map(
                            int, line.replace('"', "").split()[-2:]
                        )
    ...
    def __str__(self):
        try:
            if "elapsed_time" in self._data.keys():
                t = self._data["elapsed_time"]
            else:
                t = self._data["secs"]

            return "{} PSR {:<10} for {:>4}s at {:>4}MHz at linenumber {:>5} - {:>5}".format(
                self.command_type,
                self._data["target"],
                t,
            ...
            return "Bad command at linenumber {}-{}".format(
                self._data["start_linenumber"], self._data["end_linenumber"]
            )
...
class CIMA_Command_Parser:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("LOAD"):
                if ending_number is None and not finished_command:
                    ...
                    self.load_command(lines[starting_number : i - 1], starting_number)
                starting_number = i
                ending_number = None
            ...
            if finished_command and ending_number is not None:
                ...
                # LOAD -> PULSARON
                self.load_command(lines[starting_number:ending_number], starting_number)
                ending_number = None
    ...
    def __repr__(self):
        s = "CIMA commands from {}, using catalog {}, total time to execute: {}s\n".format(
            self.filename, self.catalog, self.total_time
        )
        return s

    def __str__(self):
        s = [
            "CIMA commands from {}, using catalog {}, total time to execute: {}s".format(
                self.filename, self.catalog, self.total_time
            )
        ]
        for c in self.commands:
        ...
        for i, line in enumerate(self.lines):
            ...
            if "CIMA session for project" in message:
                ...
                if self.data["mode"] != "pulsar":
                    logger.warning(
                        'Mode is "{}", not "pulsar"'.format(self.data["mode"])
                    )
                logger.info(
                    "CIMA session for {} with operator {} started at {}".format(
                        self.data["project"], self.data["operator"], format_time(t)
                    )
                )
            ...
            if "CIMA-load_command_file: Loaded command file" in line:
                ...
                if os.path.exists(self.data["command_file"]):
                    self.requested_commands = CIMA_Command_Parser(
                        self.data["command_file"], command_type="Request"
                    )
                    self.data["requested_commands"] = self.requested_commands.asdict()
                else:
                    logger.error(
                        'Command file "{}" does not exist'.format(
            ...
            if level in ["COMMAND", "ALERT", "ERROR"]:
                ...
                if last_command is not None and "PULSARON" in last_command:
                    ...
                    self.executed_commands[-1]._data["elapsed_time"] = (
                        self.executed_commands[-1]._data["end_time"]
                        - self.executed_commands[-1]._data["start_time"]
                    ).seconds
                    logger.info(
                        "Ended at {}".format(
                            format_time(self.executed_commands[-1]._data["end_time"])
                        )
                    )
                    logger.info(
                        "Elapsed time is {}s".format(
                            (self.executed_commands[-1]._data["elapsed_time"])
                        )
                    )
                    # increment the total observing time
                    self.data["total_time"] += self.executed_commands[-1]._data[
                        "elapsed_time"
                    ]
                    # reset the counter
                    last_command = None
    ...
    def __str__(self):
        ...
        s = [
            "### NANOGrav {} observation ({:.1f}h elapsed; {:.1f}h observing; {} scans)".format(
                self.data["project"],
                self.data["elapsed_time"] / 3600.0,
                self.data["total_time"] / 3600.0,
                len(self.executed_commands),
            )
        ]
        s.append(
            "### {} - {}".format(
                format_time(self.data["start_time"]), format_time(self.data["end_time"])
            )
        )
